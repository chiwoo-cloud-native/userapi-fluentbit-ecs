locals {
  container_name = var.container_name == null ? format("%s-%s-ecsc", var.name_prefix, var.app_name) : var.container_name
  ...
  json_object = merge(local.container_definition_without_null, var.container_definition)
  json_map = jsonencode(local.json_object)
}
